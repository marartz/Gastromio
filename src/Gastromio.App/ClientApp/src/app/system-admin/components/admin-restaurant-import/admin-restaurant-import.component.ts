...
@Component({
  ...
  styleUrls: [
    './admin-restaurant-import.component.css',
...
export class AdminRestaurantImportComponent implements OnInit {
  ...
  onSimulateRestaurants(): void {
    ...
    this.facade.importRestaurants$(this.restaurantImportFile, true).subscribe(
      (log) => {
        if (!log) return;
  ...
  onImportRestaurants(): void {
    this.logLines = undefined;
    this.facade.importRestaurants$(this.restaurantImportFile, false).subscribe(
      (log) => {
        if (!log) return;
        this.logLines = log.lines;
      },
